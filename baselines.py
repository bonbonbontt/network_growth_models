...
class RandomWalkSingleAttribute(object):
    ...
    def setup(self):
        ...
        self.chunk_nid = self.next_nid-1
        self.chunk_size = 1
        self.debug_counter = defaultdict(int)
        self.debug_list = defaultdict(list)
        ...
        for nid, attr in self.nid_attr_map.iteritems(): self.attr_nid_map[attr].append(nid)
    ...
    def add_nodes(self, chunk_seq, mean_seq, chunk_attr_sampler=None):
        ...
        for idx, (chunk_size, m) in enumerate(zip(chunk_seq, mean_seq)):
            if self.debug and (idx + 1) % chunk_debug == 0: print idx,
            self.chunk_size = chunk_size
            self.m = m
    ...
    def add_node(self, new_nid, attrs=None):
        ...
        num_marked, length, max_length = 0, 0, 1000/max(self.p_same, self.p_diff)
        ...
        while num_marked < m:
            ...
            if length > max_length:
                self.debug_counter['length_maxed'] += 1
                break
